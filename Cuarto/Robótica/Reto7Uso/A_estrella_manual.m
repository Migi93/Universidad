...
% START ALGORITHM
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Optimal_path=A_estrella_manual(MAPA, delta, pose0, posef)
...
% Calcular pistas de despegue
X0 = pose0(1);
Y0 = pose0(2);
theta0 = pose0(3);

dd = round(delta/2);
Pdx = X0 + dd*cos(theta0); % introducimos seccion recta para que el robot pueda empezar bien su camino
Pdy = Y0 + dd*sin(theta0); %Igual pero en el otro eje

%The target// Realmente esta es la posicion INICIAL no el objtivo
xval = max(Pdx, 0); %Antes se cogia del raton
yval = max(Pdy, 0);
...
%The initial position// Esta en cambio es el OBJETIVO FINAL, no la posicion
%inicial
xval = posef(1); %Establecemos el objetivo final en x
yval = posef(2); %Objetivo final en y
...
Optimal_path = [X0 Y0; Pdx Pdy; Optimal_path(2:end, :)];
