...
int main(){
    setlocale(LC_CTYPE,"Spanish");//Mostrar la Ñ
    SetConsoleOutputCP(1252);//Para mostrar el caracter €
    Empresa Yoigo;
    bool ok;
    ...
    system("PAUSE");
    ok=Yoigo.cancelaContratoTP();

    if(ok) {
        cout << "contrato cancelado" << endl;
    }
    else{
        cout << "Algun problema existe " << endl;
    }
    system("PAUSE");
    cout << endl;
    Yoigo.ver(); //tener que meter datos cada vez que pruebo el programa
    ...
    ok=Yoigo.bajaCliente(37000017); //debe eliminar el cliente y sus 3 Contratos
    ...
    cout << endl; ct1.ver(); cout << endl; ct2.ver(); cout << "\n"; ct3.ver();
    ...
    cout << *p <<"\n"<< c << endl;
    cout << ct1 <<endl<< ct2 <<"\n"<< ct3 <<"\n"<< cm1 <<"\n"<< cm2 <<endl<< cm3 << endl;
    //Esto es para ver si la clase es polimorfica
    Contrato *t[4];
