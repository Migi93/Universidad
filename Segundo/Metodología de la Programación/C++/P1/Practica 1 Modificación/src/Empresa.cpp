...
void Empresa::cargarDatos(){
    clientes[ncli++] = new Cliente(75547001, "Peter Lee", Fecha(28, 2, 2002));
    clientes[ncli++] = new Cliente(45999000, "Juan Perez", Fecha(29, 2, 2002));
...
int Empresa::nContratosMV(long int d) const{
    int conMV=0;
    for(int i=0;i<ncon;i++){
        if(contratos[i]->getDniContrato()==d){
...
int Empresa::multarClientesContratoUnico(){
    int afectados=0;
    long int dni=0;
    int anio=-1;
    for(int i=0;i<ncli;i++){
            if(clientes[i]->getFecha().getAnio()>=anio){
                anio=clientes[i]->getFecha().getAnio();
            }
    }
    for(int j=0;j<ncli;j++){
        if(anio==clientes[j]->getFecha().getAnio()){
                dni=clientes[j]->getDni();
            for(int k=0;k<ncon;k++){
            if(ContratoMovil *c = dynamic_cast <ContratoMovil*> (contratos[k])){
                    if(contratos[k]->getDniContrato()==dni && nContratosMV(dni)==1){
                c->setPrecioMinuto(c->getPrecioMinuto()+((20/100)*c->getPrecioMinuto()));
                afectados++;
                        }
                    }
                }
...
void Empresa::eliminarPeoresClientes(Empresa Vodafone){//Falta el & en el parametro
    long int dni=0;
    float suma=0;
    for(int i=0;i<Vodafone.ncli;i++){
       dni=Vodafone.clientes[i]->getDni();
       for(int j=0;j<Vodafone.ncon;j++){
        if(dni==Vodafone.contratos[j]->getDniContrato()){
            if(ContratoMovil *c = dynamic_cast <ContratoMovil*> (Vodafone.contratos[j])){
                suma=c->factura();
            }
            else if(ContratoTP *co = dynamic_cast <ContratoTP*> (Vodafone.contratos[j])){
                suma=co->factura();
            }
        }
       }
